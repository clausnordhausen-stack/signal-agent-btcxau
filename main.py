...
# CONFIG
# =========================
# Render Env Var: SECRET_KEY (recommended)
...
def _today_mt5_day_str() -> str:
    # Matches MT5 close_time day format "YYYY.MM.DD"
...
def latest(symbol: str, consumer: Optional[str] = None):
    # consumer is accepted for multi-consumer EAs; current implementation uses shared latest
    sym = norm_symbol(symbol)
    if sym not in STATE or STATE[sym]["latest"] is None:
        return {"symbol": sym, "signal": None, "updated_utc": None, "consumer": consumer}
...
@app.post("/ack")
def ack(symbol: str, updated_utc: str, consumer: Optional[str] = None):
    sym = norm_symbol(symbol)
    if not sym:
        raise HTTPException(status_code=400, detail="bad symbol")
    ...
    # store last ack string (not per-consumer in this simple implementation)
    STATE[sym]["ack"] = updated_utc

    # if ack equals current latest -> clear latest
...
def ingest_deal(d: DealEvent):
    ...
    # Optional dedup if deal_id provided
    with _db_lock:
        conn = db_conn()
        cur = conn.cursor()
...
def status_propfirma(
    ...
    daily_dd_limit: float = 350.0,   # day pnl must be > -350
    total_dd_limit: float = 600.0    # max_dd must be <= 600
):
